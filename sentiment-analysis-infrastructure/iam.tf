...
resource "aws_iam_role_policy" "batch_submitter_lambda" {
  name = "${local.name_prefix}-batch-submitter-lambda-policy"
  role = aws_iam_role.batch_submitter_lambda.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:logs:*:*:log-group:/aws/lambda/${local.name_prefix}-batch-worker:*"
      },
      {
        Effect = "Allow"
        Action = [
